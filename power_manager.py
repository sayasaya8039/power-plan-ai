...
class PowerManager:
    ...
    def set_ultimate(self) -> bool:
        """究極のパフォーマンスモードに切り替え"""
        guid = self._get_or_create_ultimate_plan()
        if guid:
            return self.set_active_plan(guid)
        return False

    def _get_or_create_ultimate_plan(self) -> Optional[str]:
        """究極のパフォーマンスプランのGUIDを取得、なければ作成"""
        # 既存のプランから検索
        plans = self.get_power_plans()
        for plan in plans:
            if "究極" in plan.name or "Ultimate" in plan.name:
                logger.info(f"究極のパフォーマンスプラン発見: {plan.guid}")
                return plan.guid
        ...
        try:
            ...
            # 出力から新しいGUIDを抽出
            match = re.search(r"GUID: ([a-f0-9-]+)", result.stdout, re.IGNORECASE)
            if match:
                new_guid = match.group(1)
                logger.info(f"究極のパフォーマンスプラン作成: {new_guid}")
                return new_guid
        except Exception as e:
